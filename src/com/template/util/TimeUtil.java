...
public class TimeUtil {
	 ...
	 */
	final static String DATE_DEFAULT_FORMAT = "yyyy-MM-dd HH:mm:ss";
	...
	/*
	 * 得到时间差
	 * 如果newDate为null就取当前时间
	 */
	public static String getTimeDifference(String newDate,String oldDate){
		SimpleDateFormat sdf = new SimpleDateFormat(DATE_DEFAULT_FORMAT);
		StringBuffer time=new StringBuffer();
		try {
			Date d1=null;
			if(newDate==null || "".equals(newDate)){
				d1=sdf.parse(TimeUtil.formatToday());
			}else{
				d1=sdf.parse(newDate);
			}
			if(d1!=null && oldDate!=null && !"".equals(oldDate)){
				Date d2=sdf.parse(oldDate);
				long diff =(d1.getTime()-d2.getTime())/1000;  //得到秒
