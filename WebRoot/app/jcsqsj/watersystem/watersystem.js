...
$(document).ready(function (){
	...
	if(haveRight('jc_watersystem_add') == false)
	{
		$('#btnAdd1').hide();
		$('#btnAdd2').hide();
	}
	
	if(haveRight('jc_watersystem_exp') == false)
	{
		$('#btnExport1').hide();
		$('#btnExport2').hide();
	}
...
function load()
{
 ...
 var status = $('#statusQuery').val();
	...
	$.get(getContextPath()+'/watersystemController/load?name='+name+'&leadername='+leadername+'&status='+status+'&',
	function(result){
		if(searchtype == 1){
...
function viewDetail(id)
{
	...
	curId = id;
	$.get(getContextPath()+"/watersystemController/get?id="+curId,
		function(result){
			var obj = jQuery.parseJSON(result);  
			if(obj.success)
			{
...
{
	curId = id;
	$('#main-content').load("./jcsqsj/watersystem/watersystemDetail.html", function () {
		$('#confirmBtn').hide();
...
function deleteData(id)
{
	$.confirm({
		...
		confirm: function(button) {
			...
			$.post(getContextPath()+"/watersystemController/delete",
			{
				id:id
			},
			function(result){
...
function exportData()
{
	...
	if(name != '')
	 ...
	 if(leadername != '')
		queryStr += "leadername like '%"+leadername+"%' AND ";
