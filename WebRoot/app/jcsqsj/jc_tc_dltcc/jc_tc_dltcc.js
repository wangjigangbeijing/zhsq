...
$(document).ready(function (){
	if(haveRight('jc_tc_dltcc_add') == false)
	{
		$('#btnAdd1').hide();
		$('#btnAdd2').hide();
	}
	
	if(haveRight('jc_tc_dltcc_exp') == false)
	{
		$('#btnExport1').hide();
		$('#btnExport2').hide();
	}
...
{
	curId = id;
	$('#main-content').load("./jcsqsj/jc_tc_dltcc/jc_tc_dltccDetail.html", function () {
		$('#confirmBtn').hide();
...
function deleteData(id)
{
	$.confirm({
		...
		confirm: function(button) {
			...
			$.post(getContextPath()+"/jc_tc_dltccController/delete",
			{
				id:id
			},
			function(result){
...
function exportData()
{
	...
	if(rateinfo != '')
		queryStr += "rateinfo = '"+rateinfo+"' AND ";
	
	var roadname = $('#roadnameQuery').val();
	if(roadname != '')
		queryStr += "roadname like '%"+roadname+"%' AND ";
