...
$(document).ready(function (){
	...
	if(haveRight('jc_shelter_add') == false)
	{
		$('#btnAdd1').hide();
		$('#btnAdd2').hide();
	}
...
function load()
{
 ...
 var status = $('#statusQuery').val();
	...
	$.get(getContextPath()+'/shelterController/load?name='+name+'&address='+address+'&status='+status+'&',
	function(result){
		if(searchtype == 1){
...
function viewDetail(id)
{
	...
	curId = id;
	$.get(getContextPath()+"/shelterController/get?id="+curId,
		function(result){
			var obj = jQuery.parseJSON(result);  
			if(obj.success)
			{
...
{
	curId = id;
	$('#main-content').load("./jcsqsj/shelter/shelterDetail.html", function () {
		$('#confirmBtn').hide();
...
function deleteData(id)
{
	$.confirm({
		...
		confirm: function(button) {
			...
			$.post(getContextPath()+"/shelterController/delete",
			{
				id:id
			},
			function(result){
