...
$(document).ready(function (){
	...
	if(haveRight('jc_xftd_add') == false)
	{
		$('#btnAdd1').hide();
		$('#btnAdd2').hide();
	}
	
	if(haveRight('jc_xftd_exp') == false)
	{
		$('#btnExport1').hide();
		$('#btnExport2').hide();
	}
...
function load()
{
	...
	var forbuildings = $('#forbuildingsQuery').val();
	
	$.get(getContextPath()+'/jc_xftdController/load?name='+name+'&forbuildings='+forbuildings+'&',
	function(result){
			if(searchtype == 1){
		...
		if(obj.success)
		{
			dataTable = $('#dataTable').dataTable( {
				...
				"columns": [
					...
					{ 'data': 'forbuildings' ,'sClass':'text-center'},
...
function viewDetail(id)
{
	...
	curId = id;
	$.get(getContextPath()+"/jc_xftdController/get?id="+curId,
		function(result){
			var obj = jQuery.parseJSON(result);  
			if(obj.success)
			{
...
{
	curId = id;
	$('#main-content').load("./jcsqsj/jc_xftd/jc_xftdDetail.html", function () {
		$('#confirmBtn').hide();
...
function deleteData(id)
{
	$.confirm({
		...
		confirm: function(button) {
			...
			$.post(getContextPath()+"/jc_xftdController/delete",
			{
				id:id
			},
			function(result){
...
function exportData()
{
	...
	var forbuildings = $('#forbuildingsQuery').val();
	if(forbuildings != '')
		queryStr += "forbuildings = '"+forbuildings+"' AND ";
