...
$(document).ready(function (){
	...
	if(haveRight('jc_volunteerteam_add') == false)
	{
		$('#btnAdd1').hide();
		$('#btnAdd2').hide();
	}
	
	if(haveRight('jc_volunteerteam_exp') == false)
	{
		$('#btnExport1').hide();
		$('#btnExport2').hide();
	}
...
function load()
{
 ...
 var address = $('#addressQuery').val();
	...
	$.get(getContextPath()+'/volunteerteamController/load?name='+name+'&regstatus='+regstatus+'&address='+address+'&',
	function(result){
		if(searchtype == 1){
...
{
	curId = id;
	$('#main-content').load("./jcsqsj/volunteerteam/volunteerteamDetail.html", function () {
		$('#confirmBtn').hide();
...
function deleteData(id)
{
	$.confirm({
		...
		confirm: function(button) {
			...
			$.post(getContextPath()+"/volunteerteamController/delete",
			{
				id:id
			},
			function(result){
