...
$(document).ready(function (){
	...
	if(haveRight('jc_pubfacilities_hj_add') == false)
	{
		$('#btnAdd1').hide();
		$('#btnAdd2').hide();
	}
	
	if(haveRight('jc_pubfacilities_hj_exp') == false)
	{
		$('#btnExport1').hide();
		$('#btnExport2').hide();
	}
...
function load()
{
 ...
 var objState = $('#objStateQuery').val();
	...
	dataGridTable = $("#dataTable").dataTable({	
		...
		"autoWidth": false,
		"bServerSide" : true,
		"sAjaxSource" : getContextPath()+'/jc_pubfacilities_hjController/load',
...
function viewDetail(id)
{
	...
	curId = id;
	$.get(getContextPath()+"/jc_pubfacilities_hjController/get?id="+curId,
		function(result){
...
{
	curId = id;
	$('#main-content').load("./jcsqsj/jc_pubfacilities_hj/jc_pubfacilities_hjDetail.html", function () {
		$('#confirmBtn').hide();
...
function deleteData(id)
{
	$.confirm({
		...
		confirm: function(button) {
			...
			$.post(getContextPath()+"/jc_pubfacilities_hjController/delete",
			{
				id:id
			},
			function(result){
