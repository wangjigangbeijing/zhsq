...
$(document).ready(function (){
	...
	if(haveRight('jc_pubfacilities_qt_add') == false)
	{
		$('#btnAdd1').hide();
		$('#btnAdd2').hide();
	}
	
	if(haveRight('jc_pubfacilities_qt_exp') == false)
	{
		$('#btnExport1').hide();
		$('#btnExport2').hide();
	}
...
function load()
{
 ...
 var objState = $('#objStateQuery').val();
	...
	$.get(getContextPath()+'/jc_pubfacilities_qtController/load?objname='+objname+'&deptname1='+deptname1+'&isincommunity='+isincommunity+'&material='+material+'&form='+form+'&objState='+objState+'&',
	function(result){
...
function viewDetail(id)
{
	...
	curId = id;
	$.get(getContextPath()+"/jc_pubfacilities_qtController/get?id="+curId,
		function(result){
			var obj = jQuery.parseJSON(result);  
			if(obj.success)
			{
...
{
	curId = id;
	$('#main-content').load("./jcsqsj/jc_pubfacilities_qt/jc_pubfacilities_qtDetail.html", function () {
		$('#confirmBtn').hide();
...
function deleteData(id)
{
	$.confirm({
		...
		confirm: function(button) {
			...
			$.post(getContextPath()+"/jc_pubfacilities_qtController/delete",
			{
				id:id
			},
			function(result){
