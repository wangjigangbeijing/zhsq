...
function load()
{
 ...
 var isofficial = $('#isofficialQuery').val();
 var status = $('#statusQuery').val();

	
	$.get(getContextPath()+'/parkingController/load?name='+name+'&address='+address+'&isofficial='+isofficial+'&status='+status+'&',
	function(result){
		...
		if(obj.success)
		{
			dataTable = $('#dataTable').dataTable( {
				...
				"columns": [
					...
					{ 'data': 'isofficial' ,'sClass':'text-center'},
					{ 'data': 'parkinglotnum' ,'sClass':'text-center'},
...
function viewDetail(id)
{
	...
	curId = id;
	$.get(getContextPath()+"/parkingController/get?id="+curId,
		function(result){
			var obj = jQuery.parseJSON(result);  
			if(obj.success)
			{
...
{
	curId = id;
	$('#main-content').load("./jcsqsj/parking/parkingDetail.html", function () {
...
function deleteData(id)
{
	$.confirm({
		...
		confirm: function(button) {
			...
			$.post(getContextPath()+"/parkingController/delete",
			{
				id:id
			},
			function(result){
