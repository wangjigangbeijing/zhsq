...
$(document).ready(function (){
	...
	if(haveRight('jc_advertisement_add') == false)
	{
		$('#btnAdd1').hide();
		$('#btnAdd2').hide();
	}
	...
	if(haveRight('jc_advertisement_exp') == false)
	{
		$('#btnExport1').hide();
		$('#btnExport2').hide();
	}
...
function load()
{
 ...
 var level = $('#levelQuery').val();

	
	$.get(getContextPath()+'/jc_advertisementController/load?name='+name+'&type='+type+'&level='+level+'&',
	function(result){
			if(searchtype == 1){
...
{
	curId = id;
	$('#main-content').load("./jcsqsj/jc_advertisement/jc_advertisementDetail.html", function () {
		$('#confirmBtn').hide();
...
function deleteData(id)
{
	$.confirm({
		...
		confirm: function(button) {
			...
			$.post(getContextPath()+"/jc_advertisementController/delete",
			{
				id:id
			},
			function(result){
