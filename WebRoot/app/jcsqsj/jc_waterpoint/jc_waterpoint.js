...
$(document).ready(function (){
	...
	if(haveRight('jc_waterpoint_add') == false)
	{
		$('#btnAdd').hide();
	}
	
	if(haveRight('jc_waterpoint_exp') == false)
	{
		$('#btnExport1').hide();
		$('#btnExport2').hide();
	}
...
function load()
{
 ...
 var type = $('#typeQuery').val();
	...
	$.get(getContextPath()+'/jc_waterpointController/load?name='+name+'&address='+address+'&type='+type+'&',
	function(result){
		...
		if(obj.success)
		{
			dataTable = $('#dataTable').dataTable( {
				...
				"columns": [
					...
					{ 'data': 'depth' ,'sClass':'text-center'},
					{ 'data': 'plan' ,'sClass':'text-center'},
...
{
	curId = id;
	$('#main-content').load("./jcsqsj/jc_waterpoint/jc_waterpointDetail.html", function () {
		$('#confirmBtn').hide();
...
function deleteData(id)
{
	$.confirm({
		...
		confirm: function(button) {
			...
			$.post(getContextPath()+"/jc_waterpointController/delete",
			{
				id:id
			},
			function(result){
