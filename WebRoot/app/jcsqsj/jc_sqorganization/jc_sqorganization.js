...
$(document).ready(function (){
	...
	if(haveRight('jc_sqorganization_exp') == false)
	{
		$('#btnExport1').hide();
		$('#btnExport2').hide();
	}
...
function load()
{
 ...
 var leadermobile = $('#leadermobileQuery').val();
...
{
	curId = id;
	$('#main-content').load("./jcsqsj/jc_sqorganization/jc_sqorganizationDetail.html", function () {
...
function deleteData(id)
{
	$.confirm({
		...
		confirm: function(button) {
			...
			$.post(getContextPath()+"/jc_sqorganizationController/delete",
			{
				id:id
			},
			function(result){
...
function exportData()
{
	...
	var persionsize = $('#persionsizeQuery').val();
	if(persionsize != '')
		queryStr += "persionsize = '"+persionsize+"' AND ";
	...
	var leadermobile = $('#leadermobileQuery').val();
	if(leadermobile != '')
		queryStr += "leadermobile like '%"+leadermobile+"%' AND ";
 
	$.post(getContextPath()+"/dataController/exportDataOfTable",
		{
			tableId:'jc_sqorganization',
			queryStr:queryStr
		},
		function(result){
