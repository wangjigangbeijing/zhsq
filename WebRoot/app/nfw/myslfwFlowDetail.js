...
function loadTemplateProcess(id){
	$.get(getContextPath()+"/flowtemplateController/getdatatemplateprocessinfo?service=mysl&dataid=" + id,
		function(result){
...
function loadprocessdata(id){
	$.get(getContextPath()+"/flowtemplateController/loadprocessdata?service=mysl&dataid=" + id,
		function(result){
			var obj = jQuery.parseJSON(result);  
			if(obj.success)
			{
