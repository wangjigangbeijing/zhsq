...
function loadTemplateProcess(id){
	$.get(getContextPath()+"/flowtemplateController/getdatatemplateprocessinfo?service=zmbl&dataid=" + id,
		function(result){
...
function loadprocessdata(id){
	$.get(getContextPath()+"/flowtemplateController/loadprocessdata?service=zmbl&dataid=" + id,
		function(result){
			var obj = jQuery.parseJSON(result);  
			if(obj.success)
			{
