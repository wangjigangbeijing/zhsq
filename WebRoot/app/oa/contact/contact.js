...
function load()
{
	$('#btnSearch').attr('disabled','disabled');
	 var contacttype = $('#contacttypeQuery').val();
 var name = $('#nameQuery').val();
 var mobile = $('#mobileQuery').val();
 var unitname = $('#unitnameQuery').val();
 var note = $('#noteQuery').val();

	
	$.get(getContextPath()+'/contactController/load?contacttype='+contacttype+'&name='+name+'&mobile='+mobile+'&unitname='+unitname+'&note='+note+'&',
	function(result){
		...
		if(obj.success)
		{
			dataTable = $('#dataTable').dataTable( {
				...
				"data":obj.list,
				"columns": [
					{ 'data': 'contacttype' ,'sClass':'text-center'},
...
function deleteData(id)
{
	$.confirm({
		...
		confirm: function(button) {
			...
			$.post(getContextPath()+"/contactController/delete",
			{
				id:id
			},
			function(result){
