...
function load()
{
 ...
 var status = $('#statusQuery').val();
	...
	$.get(getContextPath()+'/sysTelPublishController/load?title='+title+'&status='+status+'&',
	function(result){
		...
		if(obj.success)
		{
			dataTable = $('#dataTable').dataTable( {
				...
				"columns": [
					...
					{ 'data': 'publishtime' ,'sClass':'text-center'},
...
function viewDetail(id)
{
	...
	curId = id;
	$.get(getContextPath()+"/sys_tel_publishController/get?id="+curId,
		function(result){
			var obj = jQuery.parseJSON(result);  
			if(obj.success)
			{
...
function deleteData(id)
{
	$.confirm({
		...
		confirm: function(button) {
			...
			$.post(getContextPath()+"/sysTelPublishController/delete",
			{
				id:id
			},
			function(result){
